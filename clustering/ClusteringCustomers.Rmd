...
---
Customer segmentation is a common task to develope company strategoc plan. The next step is retention customer (like wise customer chunk).

This tutorial illustrates an example of doing this with R.
Credit: http://www.kimberlycoffey.com/blog/2016/8/k-means-clustering-for-customer-segmentation

data source: https://archive.ics.uci.edu/ml/datasets/Online+Retail
Note: XLConnect relies on Rjava which is superseded by **openxlsx** package.
...
```
initial steps on exploration data
```{r}
str(data)
...
dim(data)
```
filter out only invoice from 2010 Dec 09 onward at United Kingdom
```{r}
...
```
Compute recency and consider the data of most recent *purchase*

```{r}

data$recency <- as.Date("2011-12-10") - convertToDate(data$InvoiceDate)
...
```
Compute Number of invoices/year (purchases only)
```{r}
...
```
Determine a number of clusters with elbow diagram
```{r}
